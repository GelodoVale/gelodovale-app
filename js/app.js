...
function mobileNavClick(tabId) {
    ...
    // Verificar permissões RBAC (reutiliza a mesma lógica do desktop)
    const currentUserId = sessionStorage.getItem("currentUserId");
    if (currentUserId && state.users) {
        const currentUser = state.users.find(u => u.id === currentUserId);
        if (currentUser) {
            if (tabId === "precos") {
                if (currentUser.username !== "admin" && !currentUser.permissions["tab-precos"]) {
                    alert("Você não possui permissão para acessar esta seção.");
                    return;
                }
            } else {
                const hasPerm = currentUser.permissions["tab-" + tabId];
                if (hasPerm === false) {
                    alert("Você não possui permissão para acessar esta seção.");
                    return;
                }
    ...
    // Parar câmera se mudar de aba
    if (tabId !== "inventario" && window.stopQRScanner) {
        window.stopQRScanner();
    }

    // Atualizar botões da barra inferior (apenas os 4 fixos)
    ...
    // Atualizar botões do menu "Mais"
    const moreMenuItems = document.querySelectorAll(".mobile-menu-item");
    moreMenuItems.forEach(btn => {
        btn.classList.remove("active");
        if (btn.getAttribute("data-tab") === tabId) {
            btn.classList.add("active");
        }
    ...
    // Sincronizar com o desktop nav-item (sidebar)
    ...
    // Atualizar header e renderizar conteúdo
...
function toggleMobileMoreMenu() {
    ...
    const btnMais = document.getElementById("mnav-mais");
    if (!menu) return;
    const isOpen = menu.classList.contains("open");
    if (isOpen) {
        menu.classList.remove("open");
        if (overlay) overlay.classList.remove("open");
        if (btnMais) btnMais.classList.remove("active");
    } else {
